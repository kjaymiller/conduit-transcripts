...
def get_audio_url_from_episode_number(episode_number: int) -> tuple[dict[str, str], str] | None:
    ...
    title = getattr(soup.find("title"), "text").split(" - ")[0].removeprefix("Conduit #")
    description = getattr(soup.find("meta", attrs={"property": "og:description"}), "attrs")["content"]
