...
from ..models.ingestion import IngestionJob, IngestionFile, TaskStatus
...
class IngestionJobSchema(BaseModel):
    ...
    def progress(self) -> float:
        ...
        completed = sum(
            ...
            if f.status in (TaskStatus.COMPLETED, TaskStatus.FAILED)
        )
        ...
        try:
            ...
            for filename in filenames:
                file_record = IngestionFile(
                    job_id=job_id, filename=filename, status=TaskStatus.PENDING
                )
    ...
    def update_file_status(
        ...
        try:
            ...
            file_record = (
                ...
                .join(IngestionJob)
                .filter(
                    IngestionJob.job_id == job_id, IngestionFile.filename == filename
                )
                .first()
            )
            ...
            if not file_record:
                logger.warning(
                    f"File record not found for job {job_id}, file {filename}"
                )
                return
            ...
            if job:
                all_files = (
                    job.files
                )  # This uses the relationship, might need refresh if not in session?
                ...
                if all(s == TaskStatus.PENDING for s in statuses):
                    ...
                    s in (TaskStatus.COMPLETED, TaskStatus.FAILED) for s in statuses
                ):
